...
# ─── Master Live Switch — poori app ke saare background network threads
# (Fyers WS/REST + Binance WS/REST) ek hi jagah se ON/OFF karne ke liye.
# Default: True (app start hote hi jaisa pehle hota tha waisa hi auto-start —
# "auto on"). Header ka toggle icon (chart.html) isi flag ko backend HTTP
# route (/api/master_switch) ke zariye flip karta hai. Ye state sirf is
# process/session ke liye hai — kabhi disk par save nahi hoti, isliye agla
# fresh app-load hamesha wapas default ON se hi shuru hoga.
_MASTER_LIVE: dict = {"on": True}
_MASTER_LIVE_LOCK = threading.Lock()

def is_live_on() -> bool:
    with _MASTER_LIVE_LOCK:
        return bool(_MASTER_LIVE["on"])

def set_live_on(on: bool) -> None:
    with _MASTER_LIVE_LOCK:
        _MASTER_LIVE["on"] = bool(on)
    _slog(f"Master Live Switch → {'ON' if on else 'OFF'}", level=("ok" if on else "warn"))
...
def _ensure_binance_ws_threads():
    ...
    if "MasterSwitchEnforcer" not in names:
        threading.Thread(target=_master_switch_enforcer_loop, name="MasterSwitchEnforcer", daemon=True).start()
...
fyers_ws = None   # global handle — Master Live Switch enforcer isko force-close karta hai

# ─── Master-switch enforcer — jab switch OFF ho, saare open WS connections
# (Fyers + Binance) ko turant force-close karta hai, taaki OFF tap karte hi
# data flow turant ruk jaaye (reconnect loops khud check karte hain ki wapas
# connect na karein jab tak switch phir se ON na ho). ─────────────────────────
def _master_switch_enforcer_loop():
    _was_on = True
    while True:
        try:
            on = is_live_on()
            if _was_on and not on:
                # abhi-abhi OFF hua — sab kuch force-close karo
                try:
                    with _BN_WS_APPS_LOCK:
                        apps = dict(_BN_WS_APPS)
                ...
                try:
                    global fyers_ws
                    if fyers_ws is not None:
                        if hasattr(fyers_ws, "close_connection"):
                            fyers_ws.close_connection()
                        elif hasattr(fyers_ws, "close"):
                            fyers_ws.close()
                except Exception:
                    pass
            _was_on = on
    ...
    def _server_loop():
        ...
        class _Handler(http.server.BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                # ── Master Live Switch — header ke icon (chart.html, SV1+SV2
                # dono me common) isi route se poori app ke saare background
                # threads (Fyers + Binance, WS + REST) ek saath ON/OFF karta
                # hai. GET /api/master_switch → current state. GET
                # /api/master_switch?set=on|off → state change karo, aur naya
                # state wapas bhejo. ─────────────────────────────────────────
                if parsed.path == "/api/master_switch":
                    mqs = _up.parse_qs(parsed.query, keep_blank_values=False)
                    set_val = mqs.get("set", [""])[0].lower()
                    if set_val in ("on", "off"):
                        set_live_on(set_val == "on")
                    body = json.dumps({"on": is_live_on()}).encode()
                ...
                if parsed.path == "/api/market_depth":
                    ...
                    try:
                        if not is_live_on():
                            body = json.dumps({"error": "Live data paused (Master Switch OFF)",
                                                "_debug": {"branch": "master_switch_off"}}).encode()
                            self.send_response(200)
                        elif not dsymbol:
