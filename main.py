...
def fetch_bn_intraday(interval_mins: int) -> list:
    ...
    if not candles:
        # First-ever load, Supabase still empty — fall back to a direct (slower) Fyers fetch
        _days = {1: 10, 5: 30, 15: 60, 45: 90}.get(interval_mins, 30)
        today  = _ist_now().strftime("%Y-%m-%d")
        from_d = (_ist_now() - datetime.timedelta(days=_days)).strftime("%Y-%m-%d")
...
def load_bn_daily() -> list:
    ...
    if candles:
        return candles
    # First-ever load, Supabase still empty — do a direct (slower) Fyers fetch now
    _bg_refresh_bn_daily()
    return _supa.fetch_historical_candles("NSE:NIFTYBANK-INDEX", "D", limit=3000)
...
def fetch_btc(interval: str = "1m", limit: int = 1000) -> list:
    ...
    if candles:
        return candles
    # First-ever load, Supabase still empty — direct (slower) Binance fetch now
    _bg_refresh_btc(interval, limit)
...
def load_btc_daily() -> list:
    ...
    if candles:
        return candles
    # First-ever load, Supabase still empty — do a direct (slower) Binance fetch now
    _bg_refresh_btc_daily()
    return _supa.fetch_historical_candles("BTCUSDT", "1d", limit=3000)
